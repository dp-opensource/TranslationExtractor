    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($input->getOption('force') !== true && $input->getOption('dump-messages') !== true) {
            ...
            foreach ($files as $file) {
                ...
                $loader = new \TransBundle\Translation\Loader\PotFileLoader();
                $this->messages->addCatalogue($loader->load($file->getPathname(), $input->getArgument('locale'), $domain));
            }
            ...
            if($input->getOption('force') === true) {
                ...
                if ($input->getOption('output-format') == 'yml') {
                    $formatter = new \TransBundle\Translation\Formatter\YmlFormatter();
                } elseif ($input->getOption('output-format') == 'php') {
                    $formatter = new \TransBundle\Translation\Formatter\PhpFormatter();
                } elseif ($input->getOption('output-format') == 'pot') {
                    $formatter = new \TransBundle\Translation\Formatter\PotFormatter();
                } else {
                    $formatter = new \TransBundle\Translation\Formatter\XliffFormatter($input->getOption('source-lang'));
                }
                foreach ($this->messages->getDomains() as $domain) {
